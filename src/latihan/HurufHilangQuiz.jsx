...
function HurufHilangQuiz() {
  ...
  const fetchSoal = async () => {
    try {
      const res = await axios.get(
        "http://127.0.0.1:8000/api/soal/huruf-hilang"
      );
  ...
  const handleAnswer = (option) => {
    ...
    console.log(option, correctAnswer, isCorrect);
  ...
  return (
    <div className="quiz-page">
      <div className="quiz-container">
        ...
        <h1 className="quiz-title">TES HURUF HILANG</h1>
        ...
        <div className="label-grid">
          {letters.map((l, i) => (
            <div key={i} className="label-box">{l}</div>
          ))}
        </div>
...
export default HurufHilangQuiz;